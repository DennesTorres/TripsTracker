...
public class UserBusinessTests
{
    ...
    public static async Task ClassInitialize(TestContext _)
    {
        ...
        await ctx.Database.ExecuteSqlRawAsync(@"
            ...
            VALUES (0, 'orphan@system.local', GETUTCDATE(), 0, 0);
    ...
    public async Task GetStorageUsedAsync_ReturnsZero_ForNewUser()
    {
        ...
        var used = await f.Biz.GetStorageUsedAsync(user.Id);
    ...
    public async Task AddStorageUsedAsync_IncrementsStorageUsedBytes()
    {
        ...
        await f.Biz.AddStorageUsedAsync(user.Id, 5_000_000);

        var updated = await f.Ctx.Users.AsNoTracking().FirstAsync(u => u.Id == user.Id);
        Assert.AreEqual(5_000_000L, updated.StorageUsedBytes);
    }
    ...
    public async Task AddStorageUsedAsync_AccumulatesAndDecrements()
    {
        ...
        await f.Biz.AddStorageUsedAsync(user.Id, 10_000_000);
        await f.Biz.AddStorageUsedAsync(user.Id, -3_000_000);

        var updated = await f.Ctx.Users.AsNoTracking().FirstAsync(u => u.Id == user.Id);
        Assert.AreEqual(7_000_000L, updated.StorageUsedBytes);
    }
