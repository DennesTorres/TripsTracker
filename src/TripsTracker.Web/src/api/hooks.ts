...
export function useUserStatement(userId: number | null) {
  return useQuery<UserStatement>({
    queryKey: ['user-statement', userId],
    queryFn: () => apiClient.get<UserStatement>(`/api/users/${userId}/points`).then(r => r.data),
    enabled: userId !== null,
    staleTime: 30_000,
  });
}
