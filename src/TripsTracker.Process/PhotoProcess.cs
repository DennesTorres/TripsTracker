...
public class PhotoProcess : IPhotoProcess
{
    ...
    public PhotoProcess(IPlacePhotoBusiness photos, IBlobStorageService blobs, IPointsBusiness points, IUserContext userContext)
    {
        _photos = photos;
        _blobs = blobs;
        _points = points;
    ...
    public async Task<PlacePhotoDto> UploadAsync(int placeId, Stream stream, string contentType, string fileName, long sizeBytes, string? caption, CancellationToken ct = default)
    {
        ...
        var result = await _photos.CreateAsync(placeId, blobName, fileName, contentType, sizeBytes, caption, ct);
        await _points.AwardAsync(result.UserId, "photo_uploaded", 5, result.Id, "Photo", ct);
        scope.Complete();
        return result;
    }
